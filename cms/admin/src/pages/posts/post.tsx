...
export default function PostPage({ id }: Props) {
  ...
  const { mutate: publishPost } = useMutation({
    mutationFn: async (mode: "publish" | "unpublish") => {
      ...
      if (!response.ok)
        throw new Error(message || "Failed to update post status");
      return { mode, message };
    },
  ...
  return (
    <main>
      ...
      <div className="text-start flex flex-col gap-4 align-start">
        <h2>Editor</h2>
        {/* Paper. Mirrors the article surface on the site so the editor is a
            real preview; the measure comes from `.markdown` inside. */}
        <div className="scheme-light rounded-xl border border-neutral-200 bg-white px-6 py-10 sm:px-10">
          <TipTap
